...
class TestDraftLayerIdsAreTheTail:
    ...
    def test_the_model_yields_the_drafter_last(self):
        ...
        # layers -- so walk the model the way the offload cache does and check the order.
        ...
        args = DeepseekV4Args(
            max_batch_size=1, max_seq_len=4096, n_layers=2, n_hash_layers=1,
            compress_ratios=(0, 4), n_mtp_layers=1, dspark_block_size=5,
            dspark_markov_rank=32, dspark_noise_token_id=1,
            dspark_target_layer_ids=(1, 2), dspark_enabled=True,
        )
        with torch.device("meta"):
            model = Transformer(args)
        ids = [layer.layer_id for layer in iter_offload_moe_layers(model)]
        assert ids == [0, 1, 2], (
