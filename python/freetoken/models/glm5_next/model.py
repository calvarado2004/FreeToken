...
# FREETOKEN_GLM_MTP_GRAPHS=0: run the MTP draft steps and catch-ups eagerly (no draft graphs).
...
class Glm5NextMTPLayer(BaseOP):
    ...
    def forward(self, embeds: torch.Tensor, positions: torch.Tensor, prev_hidden: torch.Tensor) -> torch.Tensor:
        ...
        return self.post_moe(x, self.mlp.forward(normed))
...
class Glm5NextForCausalLM(BaseLLMModel):
    ...
    def _probe_mtp(self, input_ids: torch.Tensor, hidden: torch.Tensor) -> None:
        ...
        for lo in range(1, n - 3, 64):  # full-vocab probability sorts: keep chunks small
            ...
            expected["E[accept]"] += float(torch.minimum(p, sampling_probs(draft_logits, 1.0, 0.95)).sum())
            t0 = target[1:hi - lo + 1]
    ...
    def capture_draft_graphs(self, stream, pool, dummy_req, max_span: int, reset_moe) -> None:
        """Capture the MTP layer up to its experts for 1 .. ``max_span`` rows of one request.

        The graphs cover embedding, input projection and attention (the host-heavy part of a
        draft step); the routed experts run eagerly after replay, since the hybrid offload path
        schedules its expert fetches from the host."""
        ...
        # A private pool: these graphs replay interleaved with the target verify graphs, not in
        # capture order, which a shared pool does not allow.
        ...
        for span in range(max_span, 0, -1):
            ...
            with ctx.forward_batch(batch):
                # Warm outside the graph first (kernel autotune, lazy weight splits).
    ...
    def _mtp_run(self, batch, start: int, lo: int, hi: int, tokens: torch.Tensor, hidden: torch.Tensor):
        ...
        mtp = self.mtp_layers.op_list[0]
        # The expert kernels may write into their input; keep the graph's output intact.
