...
class Attention(BaseOP):
    ...
    def __init__(
        self, layer_id: int, args: DeepseekV4Args, compress_ratio: int | None = None, *,
    ...
    def verify_block(
        ...
        qr = q = self.q_norm.forward(self.wq_a.forward(xq))
        q = self.wq_b.forward(q).unflatten(-1, (self.n_heads, self.head_dim))
        q = rms_norm(q, None, self.eps)
        apply_rotary_emb_decode(q[..., -rd:], freqs_t)

        kv = self.kv_norm.forward(self.wkv.forward(xq))
        apply_rotary_emb_decode(kv[..., -rd:], freqs_t)
        act_quant_fp8_inplace(kv[..., :-rd], 64)
