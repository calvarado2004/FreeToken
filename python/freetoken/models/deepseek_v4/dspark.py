...
from freetoken.layers import BaseOP, LinearReplicated, OPList, RMSNorm
...
class _Table(BaseOP):
    """A bare ``weight`` tensor under its own name, so a checkpoint ``<name>.weight`` binds."""
...
class MarkovHead(BaseOP):
    ...
    def __init__(self, args: DeepseekV4Args):
        ...
        self.markov_w1 = _Table(torch.empty(args.vocab_size, self.rank, dtype=torch.float32))
        self.markov_w2 = torch.empty(args.vocab_size, self.rank, dtype=torch.bfloat16)

    def embed(self, token_ids: torch.Tensor) -> torch.Tensor:
        """``[B]`` token ids -> ``[B, r]`` Markov embedding."""
        return self.markov_w1.forward(token_ids)
...
class ConfidenceHead(BaseOP):
    ...
    def __init__(self, args: DeepseekV4Args):
        self.proj = torch.empty(1, args.dim + args.dspark_markov_rank, dtype=torch.float32)
...
class DSparkDrafter(BaseOP):
    ...
    def __init__(self, args: DeepseekV4Args, *, strategy: str = "offload", decode_target: str = "gpu", quant_config=None, prefix: str = ""):
        ...
        self.main_proj = LinearReplicated(
            self.dim * len(self.target_layer_ids), self.dim, has_bias=False,
            quant_config=quant_config, prefix=f"{prefix}.main_proj",
        )
        ...
        self.layers = OPList([
            Block(
                args.n_layers + k, args, 0, strategy=strategy, decode_target=decode_target,
                quant_config=quant_config, prefix=f"{prefix}.layers.{k}",
            )
        ...
        self.hc_head_fn = torch.empty(self.hc_mult, hc_dim, dtype=torch.float32)
        self.hc_head_base = torch.empty(self.hc_mult, dtype=torch.float32)
    ...
    def forward(self, *args, **kwargs):
        raise NotImplementedError("the drafter runs through propose() / catch_up_context()")
    ...
    def combine_target_hidden(self, aux_hidden: torch.Tensor) -> torch.Tensor:
        ...
        return self.main_norm.forward(self.main_proj.forward(aux_hidden))
    ...
    def propose(
        ...
        base_logits = target_logits_fn(self.norm.forward(head_hidden)).view(
    ...
    def sample_block(
        ...
        for k in range(gamma):
            ...
            confidence[:, k].copy_(
                self.confidence_head.forward(head_hidden[:, k], markov)
            )
            prev = next_token
