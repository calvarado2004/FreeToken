...
def shard_vocab(t: torch.Tensor) -> torch.Tensor:
    """This rank's block of vocabulary rows, in its own storage.

    Mirrors ``VocabParallelEmbedding`` exactly (``div_ceil`` rows per rank, the last rank
    short), because that class sizes its weight with the same split -- a different cut here
    would not be a wrong slice, it would simply fail to load."""
    rank, size = tp_info()
    if size == 1:
        return t
    per = div_ceil(t.shape[0], size)
    lo = rank * per
    hi = min(lo + per, t.shape[0])
    return t[lo:hi].clone(memory_format=torch.contiguous_format)
...
def validate_tp(args) -> None:
    ...
    # The vocabulary deliberately has NO divisibility check here: VocabParallelEmbedding
    # sizes its rows with div_ceil and lets the last rank come up short, and shard_vocab
    # mirrors that split. Requiring an exact divide would reject configs the framework
    # serves.


__all__ = ["div_tp", "shard", "shard_vocab", "tp_info", "tp_rank", "tp_size", "validate_tp"]
